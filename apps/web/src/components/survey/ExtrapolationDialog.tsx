...
interface ExtrapolationDialogProps {
  open: boolean;
  run: Run;
  onClose: () => void;
}
...
function TabPanel(props: TabPanelProps) {
  ...
  return (
    <div
      role="tabpanel"
      hidden={value !== index}
      id={`extrapolation-tabpanel-${index}`}
      aria-labelledby={`extrapolation-tab-${index}`}
      {...other}
    >
...
export const ExtrapolationDialog: React.FC<ExtrapolationDialogProps> = ({
  open,
  run,
  onClose,
}) => {
  ...
  const handleExtrapolateExisting = () => {
    if (!selectedSurvey) return;

    // Navigate to extrapolation page (parameters will be filled there)
    navigate(`/runs/${run.id}/extrapolation/new?surveyId=${selectedSurvey}`);
    handleClose();
  };

  const handleUploadAndExtrapolate = async () => {
    ...
    try {
      ...
      // Navigate to extrapolation page with surveyId
      // The API returns the ID at the top level or in survey_data.id
      const surveyDataId = data.id || data.survey_data?.id;
      if (surveyDataId) {
        navigate(`/runs/${run.id}/extrapolation/new?surveyId=${surveyDataId}`);
        handleClose();
      } else {
      ...
      console.error('Upload and extrapolate failed:', error);
  ...
  const canExtrapolateExisting = selectedSurvey;
  const canUploadAndExtrapolate = uploadedFile && surveyType;

  return (
    <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
      <DialogTitle>
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Typography variant="h6">Survey Extrapolation</Typography>
      ...
      <DialogContent>
        <Stack spacing={3}>
          <Alert severity="info">
            Extrapolate survey data beyond the last measured point using various methods.
          </Alert>
      ...
      <DialogActions>
        ...
        {tabValue === 0 ? (
          <Button
            variant="contained"
            onClick={handleExtrapolateExisting}
            disabled={!canExtrapolateExisting || isUploading}
          ...
          <Button
            variant="contained"
            onClick={handleUploadAndExtrapolate}
            disabled={!canUploadAndExtrapolate || isUploading}
